...
return new class extends Migration
{
    ...
    {
        if (!Schema::hasTable('career_hug_level_dates')) {
            ...
            if (!$this->hasForeignKey('career_hug_level_dates', 'ch_ld_ch_id_foreign')) {
                $existingForeignKey = $this->getExistingForeignKey('career_hug_level_dates', 'career_hug_id');
                if ($existingForeignKey) {
                    Schema::table('career_hug_level_dates', function (Blueprint $table) use ($existingForeignKey) {
                        $table->dropForeign($existingForeignKey);
                    });
                }
                
                Schema::table('career_hug_level_dates', function (Blueprint $table) {
                    $table->foreign('career_hug_id', 'ch_ld_ch_id_foreign')
                        ->references('id')
                        ->on('career_hugs')
                        ->onDelete('cascade');
