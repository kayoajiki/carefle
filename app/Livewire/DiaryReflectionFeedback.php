...
class DiaryReflectionFeedback extends Component
{
    ...
    protected function buildFeedbackPrompt(Diary $diary, string $context, $pastDiaries, int $streak = 0): string
    {
        ...
        $prompt .= "【フィードバックの形式とポイント】\n";
        $prompt .= "- 「はい、共感しました。」などの形式的な返答は避け、自然な文章で始める\n";
        $prompt .= "- 日記の内容に触れながら、具体的な気づきや成長のポイントを指摘する\n";
        $prompt .= "- 過去の日記と比較して変化や成長を感じられる点があれば伝える\n";
        if ($streak >= 3) {
            $prompt .= "- 連続記録日数に触れて、継続を自然に褒める\n";
        }
        $prompt .= "- より深い内省を促す質問を1-2個含める\n";
        $prompt .= "- 励ましの言葉を自然な形で含める\n";
        $prompt .= "- 簡潔で読みやすい文章（3-5文程度）\n";
        $prompt .= "- 親しい友人に語りかけるような、自然で温かみのある口調\n\n";
        $prompt .= "上記のポイントを踏まえて、自然で読みやすいフィードバックを生成してください。";
