...
class ResumeUploadForm extends Component
{
    ...
    {
        $this->documents = Resume::where('user_id', Auth::id())
            ->recent()
            ->get();
    }
    ...
    public function save()
    {
        ...
        try {
            // ファイルを保存
            $path = $this->pdf->store('resumes/' . Auth::id(), 'public');
            ...
            // アクティビティログに記録
            app(ActivityLogService::class)->logResumeUploaded(
                Auth::id(),
                $resume->id,
            ...
            // 成功メッセージ
            session()->flash('message', '履歴書をアップロードしました。');
    ...
    public function delete($id)
    {
        ...
        session()->flash('message', '履歴書を削除しました。');
        $this->loadDocuments();
    }

    public function render()
    {
        return view('livewire.resume-upload-form');
    }
}
